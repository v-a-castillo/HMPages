...
export const useTableActions = (columnsProps: Column[] = [], dataInfo: any = []) => {
  ...
  const [columnsConf, setColumnsConf] = useState<Column[]>(columnsProps);
  ...
  const handlerColumView = (columns: Column[]) => {
    setColumnsConf(columns);
  }

  const visibleColumns = columnsConf.filter((col) => col.visible);
